...
import uu.farmbio.sg.types.SignatureRecordDecision
...
private[vs] object Sdf2LibSVM {
  ...
  def sdf2signatures = (poses: String, index: Long, molCount: Long) => {
    ...
    while (it.hasNext()) {
      ...
      val sig = (index, SGUtils.atom2SigRecordDecision(mol, label.toDouble, 1, 3)) // Using new method from Staffan new library
      res = res ++ Seq(sig) 
